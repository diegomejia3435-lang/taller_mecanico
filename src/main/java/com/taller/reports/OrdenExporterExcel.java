...
public class OrdenExporterExcel {
    ...
    public OrdenExporterExcel(List<OrdenMantenimiento> listOrdenes) {
        this.listOrdenes = listOrdenes;
        workbook = new XSSFWorkbook();
    }

    private void writeHeaderLine() {
        sheet = workbook.createSheet("Ordenes");
        ...
        createCell(row, 2, "Placa Vehículo", style);
        createCell(row, 3, "Mecánico", style);
        createCell(row, 4, "Fecha Ingreso", style);
        createCell(row, 5, "Fecha Salida", style);
        createCell(row, 6, "Estado", style);
        createCell(row, 7, "Diagnóstico", style);
    }
    ...
    private void writeDataLines() {
        ...
        for (OrdenMantenimiento o : listOrdenes) {
            ...
            createCell(row, columnCount++, o.getFechaSalida() != null ? o.getFechaSalida().format(formatter) : "-", style);
            createCell(row, columnCount++, o.getEstado() != null ? o.getEstado().name() : "", style);
            createCell(row, columnCount++, o.getDiagnosticoTecnico() != null ? o.getDiagnosticoTecnico() : "", style);
        }
    }
