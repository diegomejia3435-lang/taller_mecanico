...
public class HistorialExporterPDF {
    ...
    public HistorialExporterPDF(List<OrdenMantenimiento> listOrdenes) {
    ...
    private void writeTableHeader(PdfPTable table) {
        ...
        cell.setPhrase(new Phrase("Vehículo Placa", font));
        table.addCell(cell);
    ...
    public void export(HttpServletResponse response) throws IOException {
        ...
        Paragraph p = new Paragraph("Reporte de Historial de Servicios (Financiero)", font);
        p.setAlignment(Paragraph.ALIGN_CENTER);
        document.add(p);

        PdfPTable table = new PdfPTable(6);
        table.setWidthPercentage(100f);
        table.setWidths(new float[] {1.0f, 2.5f, 2.0f, 3.0f, 3.0f, 2.0f});
